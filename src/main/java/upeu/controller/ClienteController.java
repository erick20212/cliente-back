...
public class ClienteController {
	...
	public ResponseEntity<Cliente> create(@Valid @RequestBody Cliente obj){
		try {
			Cliente objeto = service.create(obj);
	...
	public ResponseEntity<Cliente> getClienteId(@PathVariable Long id){
		try {
			Cliente objeto = service.read(id).get();
			return new ResponseEntity<>(objeto, HttpStatus.CREATED);
		} catch (Exception e) {
			// TODO: handle exception
			return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
	...
	@PutMapping("/{id}")
	public ResponseEntity<Cliente> updateCliente(@PathVariable Long id, @Valid @RequestBody Cliente obj) {
	    Optional<Cliente> objeto = service.read(id);
	    if (objeto.isPresent()) {
	        // Aquí se asume que el servicio actualiza el objeto y lo retorna
	    	Cliente actualizado = service.update(obj);
